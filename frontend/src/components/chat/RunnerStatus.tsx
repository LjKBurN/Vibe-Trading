...
function formatRelative(value: string | number | null | undefined, t: (key: string, opts?: Record<string, unknown>) => string): string {
  if (value == null || value === "") return t("runnerStatus.never");
  const then = typeof value === "number"
    ? (value < 1_000_000_000_000 ? value * 1000 : value)
    : new Date(value).getTime();
  if (!Number.isFinite(then)) return t("runnerStatus.unknown");
  const deltaSec = Math.round((Date.now() - then) / 1000);
  if (deltaSec < 0) return t("runnerStatus.justNow");
  if (deltaSec < 60) return t("runnerStatus.secondsAgo", { seconds: deltaSec });
  if (deltaSec < 3600) return t("runnerStatus.minutesAgo", { minutes: Math.floor(deltaSec / 60) });
  if (deltaSec < 86_400) return t("runnerStatus.hoursAgo", { hours: Math.floor(deltaSec / 3600) });
  return t("runnerStatus.daysAgo", { days: Math.floor(deltaSec / 86_400) });
}

function formatCountdown(iso: string | undefined, t: (key: string, opts?: Record<string, unknown>) => string): { label: string; expired: boolean; soon: boolean } {
  if (!iso) return { label: "—", expired: false, soon: false };
  const target = new Date(iso).getTime();
  if (!Number.isFinite(target)) return { label: t("runnerStatus.unknown"), expired: false, soon: false };
  const deltaSec = Math.round((target - Date.now()) / 1000);
  if (deltaSec <= 0) return { label: t("runnerStatus.expired"), expired: true, soon: false };
...
function summarizeLimits(limits: LiveMandateLimits | undefined, t: (key: string, opts?: Record<string, unknown>) => string): string {
  if (!limits) return "";
  const parts: string[] = [];
  if (limits.max_order_notional_usd != null) parts.push(`≤${formatUsd(limits.max_order_notional_usd)}/${t("runnerStatus.perOrder")}`);
  if (limits.max_trades_per_day != null) parts.push(`${limits.max_trades_per_day}/${t("runnerStatus.perDay")}`);
  if (limits.max_leverage != null) parts.push(limits.max_leverage <= 1 ? t("runnerStatus.noLeverage") : `${limits.max_leverage}×`);
  return parts.join(" · ");
}

function fallbackAuthorizeInstruction(t: (key: string) => string): string {
  return t("runnerStatus.authorizeInstruction");
}
...
function BrokerRow({
  ...
  const authorizeInstruction = authorizeHint?.instruction
    ?? (authorizeFailed ? fallbackAuthorizeInstruction(t) : t("runnerStatus.loadingAuth"));
  const authorizeNote = t("runnerStatus.authorizeNote");
  ...
  const toggleRunner = useCallback(async () => {
    ...
    try {
      if (runnerAlive) {
        ...
        toast.success(t("runnerStatus.runnerStopped", { broker: brokerKey }));
      } else {
        await api.startLiveRunner(brokerKey);
        toast.success(t("runnerStatus.runnerStarted", { broker: brokerKey }));
      }
      onRefresh();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("runnerStatus.runnerControlFailed"));
    } finally {
      setBusy(false);
    }
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {mandate.expires_at && (
                  <span
                    ...
                    title={t("runnerStatus.expiresAt", { date: new Date(mandate.expires_at).toLocaleString() })}
                  >
                    <Clock className="h-2.5 w-2.5" />
                    {countdown.expired ? t("runnerStatus.expired") : t("runnerStatus.expiresIn", { time: countdown.label })}
                  </span>
                )}
              </div>
              <div className="mt-0.5 font-mono text-[11px] text-foreground">
                {summarizeLimits(mandate.limits, t) || t("runnerStatus.limitsUnavailable")}
              </div>
            </div>
          ) : (
            <div className="rounded-md border border-dashed bg-background/40 p-2 text-[10px] text-muted-foreground">
              {t("runnerStatus.noMandate")}
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-2">
            {halted ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-destructive">
                ...
                {t("runnerStatus.haltedControlsDisabled")}
              </span>
            ) : (
              <span className="text-[10px] text-muted-foreground">
                {runnerAlive ? t("runnerStatus.runtimeActive") : t("runnerStatus.idle")}
              </span>
            )}
            <button
              type="button"
              ...
              title={runnerAlive ? t("runnerStatus.stopRunner") : t("runnerStatus.startRunner")}
            >
              {busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Power className="h-3 w-3" />}
              {runnerAlive ? t("runnerStatus.stopRunnerBtn") : t("runnerStatus.startRunnerBtn")}
            </button>
          </div>
        </>
      )}
  ...
  return (
    <div className="grid gap-2">
      <button
        ...
        aria-label={t("runnerStatus.connectorRuntimeStatus")}
        aria-expanded={open}
      >
        <Activity className="h-3 w-3 shrink-0" />
        <span className="shrink-0">{t("runnerStatus.connectorRuntime")}</span>
        <span className="truncate text-muted-foreground">
          {authorizedCount > 0 ? t("runnerStatus.connectedCount", { count: authorizedCount }) : t("runnerStatus.noConnectorConnected")}
        </span>
        {anyRunning && !isHalted && (
          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-600 dark:text-emerald-400">
            <CircleDot className="h-2.5 w-2.5" />
            {t("runnerStatus.running")}
          </span>
        )}
        {isHalted && (
