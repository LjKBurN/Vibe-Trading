...
export const ThinkingTimeline = memo(function ThinkingTimeline({ messages, isLatest = false }: Props) {
  ...
  const toolLabel = (tool?: string): string => {
    if (!tool) return t("thinkingTimeline.processing");
  ...
  const summaryText = isRunning
    ? t("thinkingTimeline.runningTool", { tool: toolLabel(latestTool) })
    : t("thinkingTimeline.doneSteps", { count: stepCount, seconds: totalMs > 0 ? t("thinkingTimeline.secondsSuffix", { s: (totalMs / 1000).toFixed(1) }) : "" });
        ...
        <div className="border-t border-border/30 px-3 py-1.5 space-y-0.5">
          {steps.map((step, i) => (
            <div key={`${step.tool}-${i}`} className="flex items-center gap-2 py-1 text-xs">
              ...
              {/* Duration or status */}
              {step.status === "running" ? (
                <span className="text-[10px] text-primary/60">{t("thinkingTimeline.running")}</span>
