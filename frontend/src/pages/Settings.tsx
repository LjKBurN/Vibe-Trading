    ...
    Promise.all([api.getLLMSettings(), api.getDataSourceSettings()])
      ...
      .catch((error) => {
        ...
        if (isAuthRequiredError(error)) {
          ...
          toast.error(t("settings.failedLoadLLM", { error: message }));
          toast.error(t("settings.failedLoadDataSource", { error: message }));
        }
      })
      .finally(() => {
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{t("settings.desc")}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? t("settings.oauthHint", { command: selectedProvider.login_command })
        : t("settings.noApiKeyNeeded");
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <select
                ...
                className={fieldClass}
              >
                <option value="">{t("settings.reasoningOff")}</option>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{t("settings.dataSourceDesc")}</p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            ...
            >
              ...
              {dataSaving ? t("settings.saving") : t("settings.saveDataSource")}
            </button>
          </div>
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                {dataSettings.baostock_installed
                  ? t("settings.packageInstalled")
                  : t("settings.packageNotInstalled")}
              </p>
            </div>
          </div>
        </div>
